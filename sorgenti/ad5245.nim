...
proc setValue*(self: Ad5245, data: uint8)
proc setInstruction*(self: Ad5245, instruction: uint8)
proc setResWA*(self: Ad5245, ohmValue: int)
proc setResWB*(self: Ad5245, ohmValue: int)
proc setVoltage*(self: Ad5245, vOut: float, voltA: float, voltB: float = 0.0)
...
proc setValue*(self: Ad5245, data: uint8) = #set the value 0 = RESmin, 255 = RESmax
  self.writeAd5245(data) #call proc for write in Ad5245
  
proc setResWA*(self: Ad5245, ohmValue: int) = #proc to set the value of the nominal resistance between B and W
  var copyOhmValue = ohmValue
  if copyOhmValue > self.resValue:
    copyOhmValue = self.resValue
    self.writeAd5245(self.calculateValWA(copyOhmValue))
  elif copyOhmValue <= 0:
    self.setInstruction(32)
  else:
    self.writeAd5245(self.calculateValWA(copyOhmValue))

proc setResWB*(self: Ad5245, ohmValue: int) = #proc to set the value of the nominal resistance between A and W
  var copyOhmValue = ohmValue
  if copyOhmValue > self.resValue:
    copyOhmValue = self.resValue
    self.writeAd5245(self.calculateValWB(copyOhmValue))
  elif copyOhmValue <= 0:
    self.setInstruction(32)
  else:
    self.writeAd5245(self.calculateValWB(copyOhmValue))

proc setVoltage*(self: Ad5245, vOut: float, voltA: float, voltB: float = 0.0) = #proc for set the voltage (partiton of V).
  var copyVout = vOut
  if copyVout >= (voltA-voltB):
    copyVout = voltA-0.1
  var value = uint8(((-256*copyVout)-(256*voltA))/(voltB-voltA))
